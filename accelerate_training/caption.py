...
@torch.no_grad()
def evaluation(model, data_loader, tokenizer, device, config, accelerator=None, max_length=30) :
    # test
    model.eval()
...
def main(args, config):
    ...
    for epoch in range(start_epoch, max_epoch):
        ...
        valid_results = evaluation(model, val_loader, tokenizer, device, config, accelerator=accelerator, max_length=max_length) 
    ...
    vqa_result = evaluation(model, test_loader, tokenizer, device, config, accelerator=accelerator, max_length=max_length)    
    dist.barrier()   
